...
// variables for serial communcation
char commandbyte;
String command;
...
void move_to_starting_position() {
  if (status_variable == 3) {
    ...
    Serial.println("You may now enter a new measurement interval in milliseconds...");
    Serial.print("\n");    
...
void loop() {
  ...
  if (Serial.available() > 0) {
      ...
      // identify ourselves
      if (command == "*IDN?") {
        Serial.print("MoSES Target Mover\r\n");
      }

      // stop the measurement
      if (command == "Stop") {
        if (status_variable == 1 || status_variable == 2) {
          status_variable = 3; // returning
          move_to_starting_position();
        }
      }

      // read parameters if we are not measuring
      if (status_variable == 0) {
        int equal_sign = command.indexOf('=');
        String parameter = command.substring(0, equal_sign);
        String value = command.substring(equal_sign + 1);
  ...
  if (digitalRead(start_Switch) == LOW) {
    if (status_variable == 0) {
      ...
      status_variable = 3; // returning
      Serial.println("-----------------------------------------------");
      Serial.println("Aborting measurement!");
      Serial.println("-----------------------------------------------");
      move_to_starting_position();
    }
  }
